...
<div class="dashboard">
    <div class="container ">
        <div class="grid grid-cols-1 gap-8 mt-8 lg:grid-cols-4 xl:grid-cols-5 md:grid-cols-3">
            @foreach ($intrests as $l)
            <div>
                <div class="w-full member-card">
                    <img class="w-full h-32" src="/storage/{{$l->event->image}}" alt="{{$l->event->title}}">
                    <div class="p-1 border-green-600 border-1">
                        <h2 class="text-xl uppercase title">
                            {{$l->event->title}}
                        </h2>
                        <p>
                            {{ Str::limit(strip_tags($l->event->description), 200) }}
                        </p>
                        <p class="text-xl font-black text-left text-red-700 ">
                            @if ($l->event->price)
                            <span>{{$currency}} {{$l->event->price}} </span>
                            @else
                            <span>Free</span>
                            @endif
                        </p>

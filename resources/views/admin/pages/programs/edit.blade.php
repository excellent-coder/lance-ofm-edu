    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form method="POST" @submit.prevent="submit($event)" action="{{route('admin.programs.update', $program->id)}}">
                            @csrf
                            <div class="row">
                                <div class="col-12 col-lg-8">
                                    <div class="form-group">
                                        <label>Short Description</label>
                                        <textarea placeholder="excerpt" class="form-control required" name="excerpt" rows="3">{{$program->excerpt}}</textarea>
                                    </div>
                                    <div class="form-group">
                                        <label for="body">Long Description</label>
                                        <textarea class="form-control tinymce" name="description" rows="12">{{$program->description}}</textarea>
                                    </div>

                                     <div class="mt-4 row justify-content-between">
                                        <div class="my-4 checkbox checkbox-primary">
                                            <input id="is_program" {{$program->is_program?'checked':''}} type="checkbox" class="form-check-input form-control"
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select id="visibility" class="form-control select2" name="visibility">
                                                <option value="1" {{$program->visibility==1?'selected':''}}>All Studenst</option>
                                                <option value="2" {{$program->visibility==2?'selected':''}}>Program Students Only</option>
                                                <option value="3" {{$program->visibility==3?'selected':''}}>Schort Course Studenst Only</option>
                                            </select>
                                            <small class="form-text text-muted">
                                ...
                                <div class="col-12 col-lg-4">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control required" type="text"
                                            name="title" value="{{$program->title}}" autocomplete="off" placeholder="program">
                                    </div>
                                    <div class="form-group">
                                        <label>Abbreviation</label>
                                        <input class="form-control required" type="text"
                                            name="abbr" value="{{$program->abbr}}" autocomplete="off" placeholder="abbreviation">
                                    </div>
                                    <div class="checkbox checkbox-primary p-t-0">
                                        <input id="active" {{$program->active?'checked':''}} type="checkbox" class="form-check-input form-control"
                                            name="active" value="1">
                                        <label for="active">
                                            Active
                                        </label>
                                    ...
                                    <div class="my-3 form-group">
                                        ...
                                        <a href="{{route('admin.levels')}}" target="_blank" rel="noopener noreferrer" title="add new Level">Add Levela</a>
                                       <select data-placeholder="maximum level" name="max_level"  id="max_level" class=" form-control select2">
                                           @foreach ($levels as $l)
                                               <option value="{{$l->level}}" {{$l->level == $program->max_level?'selected':''}}>{{$l->name}}</option>
                                           @endforeach
                                       </select>
                                    </div>
                                    <div class="form-group">

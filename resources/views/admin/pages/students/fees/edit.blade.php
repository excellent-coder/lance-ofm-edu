    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        <form method="POST" action="{{route('admin.students.results.update', $result->id)}}" id="resForm">
                            @csrf
                                <div class="row">
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        ...
                                        <select data-placeholder="Session" id="session" class="form-control select2" name="session">
                                            ...
                                            <option value="{{$p->id}}" {{$p->id == $result->session_id?'selected':''}}>{{$p->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        ...
                                        <select data-placeholder="Program" id="program" class="form-control select2" name="program">
                                            ...
                                            <option value="{{$p->id}}" {{$p->id == $result->program_id ?'selected':''}}>{{$p->abbr}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        ...
                                        <select data-placeholder="Level" id="level" class="form-control select2" name="level">
                                            ...
                                            @foreach ($levels as $p)
                                            <option value="{{$p->id}}" {{$p->id == $result->level_id ?'selected':''}}>{{$p->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        ...
                                        <select data-placeholder="Course" id="course" class="form-control select2" name="course">
                                            ...
                                            <option value="{{$p->id}}" {{$p->id == $result->course_id ?'selected':''}}>{{$p->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        ...
                                        <select data-placeholder="Short Course Student" id="student" class="form-control select2" name="student">
                                            ...
                                            <option value="{{$p->id}}" {{$p->id == $result->student_id ?'selected':''}}>{{$p->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                       ...
                                       <input value="{{$result->score}}" type="text" required id="score" class=" form-control" name="score" placeholder="score %">
                                    </div>
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        <label for="scs">Exam Date</label>
                                       <input type="date" value="{{$result->exam_date}}" class=" form-control wtk" name="exam_date">
                                    </div>
                                    <div class="form-group col-12 col-md-6 col-lg-4">
                                        <label>&nbsp;</label>
                                         <div class="checkbox checkbox-primary">
                                             <input id="active" {{$result->retaken?'checked':''}} type="checkbox" class="form-check-input form-control"
...
function fetchData(ele){
    ...
    $('#resForm').on('submit', function (e) {
        e.preventDefault()
        isLoading(1)
        axios.post("{{route('admin.students.results.update', $result->id)}}", new FormData(this)).then(res => {
            isLoading(0)
            let data = res.data;
            if (data.status == 200) {
                ...
                if(data.type == 'success'){
                    ...
                    $('#score').val('');
                    window.location.href = "{{route('admin.students.results')}}"
                }
            }
        }).catch(err => {
              isLoading(0)
